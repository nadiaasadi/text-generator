const dummyText = [
  ...
  let generatBtnElem=document.querySelector('.btn')
  generatBtnElem.addEventListener('click',function(e){
    e.preventDefault()
    let amountInputValue=amountInputElem.value

    let randomElem=Math.floor(Math.random()*dummyText.length)
    if(amountInputValue<0 || amountInputValue>9 || amountInputValue=='' || isNaN( amountInputElem.innerHTML=='')){
resultElem.innerHTML=dummyText[randomElem]
    }
 else{
    let sliceArry=dummyText.slice(0,amountInputValue)
    let finalArray=sliceArry.map(function(item){
        return '<p>'+item+'</p>'
    }).join()
    resultElem.innerHTML=finalArray
 }
  })
